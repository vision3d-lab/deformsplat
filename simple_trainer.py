...
from util.utils import AppearanceOptModule, CameraOptModule, knn, rgb_to_sh, set_random_seed
from util.lib_bilagrid import (
    BilateralGrid,
    slice,
    color_correct,
...
class Runner:
    ...
    def train_drag(self, *args, **kwargs):
        raise NotImplementedError("Use DeformRunner.train_drag in deform_splat.py")
    ...
    def eval(self, step: int, stage: str = "val", is_pred_camtoworld=False):
        ...
        if world_rank == 0:
            ...
            color_log(0000, f"step={step} - PSNR: {stats['psnr']:.3f}, SSIM: {stats['ssim']:.4f}, LPIPS: {stats['lpips']:.3f} ")
            print(
...
def main(local_rank: int, world_rank, world_size: int, cfg: Config):
    ...
    if cfg.single_finetune and not cfg.finetune_with_only_rgb:
        raise Exception("Use DeformRunner.train_drag in deform_splat.py")
    else:
